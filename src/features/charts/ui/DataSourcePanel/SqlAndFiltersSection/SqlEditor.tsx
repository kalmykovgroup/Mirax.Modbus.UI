...
type Props = {
  sql: SqlFilter | null;
  onChangeImmediate: (next: SqlFilter | null) => void; // on typing
  onCommit: (next: SqlFilter | null) => void;           // on blur
};

export function SqlEditor({ sql, onChangeImmediate, onCommit }: Props) {
  const setSqlText = (text: string, commit: boolean) => {
    const trimmed = (text || '').trim();
    const next = trimmed.length ? { whereSql: trimmed } : null;
  ...
  return (
    <div style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ fontSize: 12, opacity: .8 }}>
        Плейсхолдеры <code>{`{{key}}`}</code> — параметры добавятся при завершении ввода (при потере фокуса), а не по каждой клавише.
      </div>
    </div>
  );
}
