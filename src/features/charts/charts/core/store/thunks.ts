...
>('charts/fetchMultiSeriesRaw', async (request, { getState, dispatch }) => {
    ...
    const shared = await dispatch(fetchMultiSeries({
        request: request,
    ...
    const bucketListDesc = buildLowerOrEqualBucketsMs(shared.bucketMilliseconds, cfg.niceMilliseconds);
    ...
    for (const f of request.template.selectedFields) {
        dispatch(
            updateView({
                ...
                currentBucketsMs: shared.bucketMilliseconds, //Устанавливаем BucketMs, по которому можно определить текущий уровень
    ...
    const snapped = snapToBucketMs(currentRange.from, currentRange.to, shared.bucketMilliseconds);

    const series: MultiSeriesItemDto[] = shared.series ?? [];

    for (const s of series) {
        dispatch(
            upsertTiles({
                field: s.field.name,
                bucketMs: shared.bucketMilliseconds,
...
>('charts/fetchMultiSeriesRaw', async ({ request, field, skipDebounce = false, skipCoverageCheck = false }, { getState, dispatch }) => {
    ...
    if (!skipCoverageCheck && view.seriesLevel && request.from && request.to) {
        ...
        if (coverageCheck.hasSufficientCoverage && coverageCheck.quality !== 'exact') {
            console.log(
                `[fetchMultiSeriesRaw] Using ${coverageCheck.quality} shared ` +
    ...
    try {
                ...
                if (!skipCoverageCheck) {
                    ...
                    for (const syncField of fieldsToLoad) {
                        ...
                        if (!syncCoverageCheck.hasSufficientCoverage ||
                            ...
                            allHaveCoverage = false;
                            console.log(
                                `[fetchMultiSeriesRaw] Sync field ${syncField.name} needs shared: ` +
        ...
        console.error(`[fetchMultiSeriesRaw] Error loading shared for ${fieldName}:`, error);
...
const fetchMultiSeries = createAsyncThunk<
    ...
    { state: RootState }
>('charts/fetchMultiSeriesRaw', async (shared, { dispatch }) => {

    const{ request, loadingType, loadingMessage } = shared;
